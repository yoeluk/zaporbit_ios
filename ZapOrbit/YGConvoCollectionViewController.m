...
- (void)viewDidLoad
{
	...
	self.toUser = [YGUser userWithDictionary:[self.details objectForKey:@"toUser"]];
	self.me = [self.details objectForKey:@"me"];
	self.convid = [self.details objectForKey:@"convid"];
	self.convo = [self.details objectForKey:@"convo"];
	self.conversation = [self.convo objectForKey:@"conversation"];
	self.messages = [self.details objectForKey:@"messages"];
	self.conversations = [self.details objectForKey:@"allConversations"];
...
-(void)messagesHeights {
	...
	for (NSDictionary *message in self.messages) {
		...
		messageLabel.attributedText = [[NSAttributedString alloc] initWithString:[message objectForKey:@"message"] attributes:attrs];
		[messageLabel sizeToFit];
		[messagesHeights addObject:[NSNumber numberWithFloat:messageLabel.bounds.size.height]];
	}
}
...
-(void)changetoConversation:(UIBarButtonItem *)sender {
	...
	if (sender.tag == 9 && convoIndex+1 < self.conversations.count) {
		self.convo = [self.conversations objectAtIndex:convoIndex+1];
	...
	self.conversation = [self.convo objectForKey:@"conversation"];
	NSDictionary *user1 = [self.convo objectForKey:@"user1"];
	NSDictionary *user2 = [self.convo objectForKey:@"user2"];
	if ([[user1 objectForKey:@"id"] longLongValue] == self->userInfo.user.id) {
		self.toUser = [YGUser userWithDictionary:user2];
	} else self.toUser = [YGUser userWithDictionary:user1];
	self.convid = [self.conversation objectForKey:@"id"];
	
	self.messages = [self.conversation objectForKey:@"messages"];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
	return CGSizeMake(305, [[self->messagesHeights objectAtIndex:self.messages.count-indexPath.row-1] floatValue]+110);
}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
	...
	if ([collectionViewHeather viewWithTag:10]) {
		[(UILabel *)[collectionViewHeather viewWithTag:10] setText:[self.conversation objectForKey:@"title"]];
	}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
	...
	switch (indexPath.section) {
		case 0:
			...
			message = [self.messages objectAtIndex:self.messages.count-indexPath.row-1];
			
			[(UILabel *)[cell.contentView viewWithTag:12] setText:[[message objectForKey:@"senderid"] longLongValue] == self.me.id ? self.me.fullName : self.toUser.fullName];
			dateString = [message objectForKey:@"created_on"];
			...
			messageView.text = [NSString stringWithFormat:@"\u00BB %@", [message objectForKey:@"message"]];
			...
			if ([[message objectForKey:@"senderid"] longLongValue] == self.me.id) {
				if (self->mePicImage) {
			...
			} else if ([[message objectForKey:@"senderid"] longLongValue] == self.toUser.id) {
				if (self->uPicImage) {
					[[[cell.contentView viewWithTag:66] viewWithTag:55] removeFromSuperview];
			...
			if ([[message objectForKey:@"senderid"] longLongValue] == self.toUser.id) {
...
- (IBAction)replyToConvo:(id)sender {
	...
	NSDictionary *details = [[NSDictionary alloc] initWithObjectsAndKeys:self.toUser, @"toUser", self.me, @"me", self.convid, @"convid", [self.conversation objectForKey:@"title"], @"title", nil];
	[composerNav.childViewControllers[0] setDetails:details];
	[composerNav.childViewControllers[0] setReplying:[NSNumber numberWithInt:1]];
